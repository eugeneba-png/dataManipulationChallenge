...
because if they are not the same data type, the comparison will not work.
...
if you compare them as strings, the result will be "04-11-2025" > "05-20-2024", which is not correct.
...
the lateStatement is not being assigned correctly because the if statement is not being executed.
...
the difference is that .query() is a more readable way to query the data, and [df['is_late'] == True] is a more verbose way to query the data.
- Which approach is more readable and why?
.query() is more readable because it is a more concise way to query the data.
...
the difference is that .drop(columns=['quantity']) is a more concise way to drop the quantity column, and .filter() is a more verbose way to drop the quantity column.
- When would you choose to drop columns vs filter to keep specific columns?
you would choose to drop columns if you want to remove the column from the dataframe, and you would choose to filter if you want to keep the column in the dataframe.
...
if you use .dropna() without specifying `subset`, it will drop all rows with any missing values in any column.
if you use .dropna(subset=['plannedShipDate', 'actualShipDate']), it will drop all rows with missing values in the plannedShipDate or actualShipDate columns.
- Why might you want to be selective about which columns to check for missing values?
you might want to be selective about which columns to check for missing values if you only want to drop rows with missing values in certain columns.
:::
...
the difference is that ascending=False will sort the data in descending order, and ascending=True will sort the data in ascending order.
- How would you sort by multiple columns (e.g., first by `is_late`, then by `days_late`)?
you would sort by multiple columns by using the following code:
```python
sorted_by_multiple_columns = shipments_with_lateness.sort_values(['is_late', 'days_late'], ascending=[False, True])
```
**Question 2: Index Management**
- Why do we use `.reset_index(drop=True)` after sorting?
we use .reset_index(drop=True) after sorting to reset the index of the dataframe to be sequential.
- What happens to the original index when you sort? Why might this be problematic?
the original index is lost when you sort the dataframe.
this is problematic because you may need to use the original index for other operations.
:::
...
sum() works on boolean values because it counts the number of True values in the column.
it counts the number of True values in the column.
:::
...
because left join will keep all the rows from the left dataframe, and only keep the rows from the right dataframe that have matching values in the key column.
- How can you check if any shipments were lost during the merge?
you can check if any shipments were lost during the merge by using the following code:
```python
shipments_with_category = shipments_with_category[shipments_with_category['prodCategory'].notna()]
```

**Question 2: Key Column Matching**
- What happens if there are duplicate `partID` values in the `product_line_df`?
if there are duplicate `partID` values in the `product_line_df`, the merge will only keep the first occurrence of the `partID` value.
:::
...
we need to use .agg() after grouping to apply a function to the grouped data.
if you don't use .agg(), the grouped data will not be aggregated.
...
grouping by `['shipID', 'prodCategory']` will answer the question of how often do orders include products from more than one category.
grouping by `'prodCategory'` alone will answer the question of how often do orders include products from a particular category.
:::
...
this comprehensive analysis answers the question of how often do orders include products from more than one category.
...
ZappTech's management can gain insights from knowing the percentage of multi-category shipments to improve their service level.
:::
